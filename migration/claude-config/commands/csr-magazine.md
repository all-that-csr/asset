...
10. **Figma 썸네일 이미지 생성**: 아래 Figma 썸네일 생성 가이드에 따라 Figma에 376x460 썸네일 프레임을 만드세요. 사용자가 Figma에서 바로 PNG로 Export할 수 있습니다.
...
매거진 생성 시 `thumbnail.html`과 함께 **Figma 파일**로도 썸네일을 만든다. 사용자가 Figma에서 직접 PNG Export하여 카카오톡 공유 시 일관된 크기의 이미지를 사용할 수 있다.

### 생성 절차
1. `mcp__claude_ai_Figma__whoami`로 계정/팀 정보 확인
2. `mcp__claude_ai_Figma__create_new_file`로 "[CAUSEWORKS]" 팀에 새 Figma 파일 생성 (파일명: `올댓CSR 매거진 Vol.[호수] 썸네일`)
3. `mcp__claude_ai_Figma__use_figma`로 376x460 프레임 생성 (아래 구조)
4. `mcp__claude_ai_Figma__upload_assets`로 배경 이미지를 Unsplash에서 다운로드 후 업로드
5. `mcp__claude_ai_Figma__get_screenshot`으로 결과 확인

### Figma 프레임 구조 (376x460) — v2 카카오풍 (라운드 칩)
```
프레임 "Vol.[호수] Thumbnail" (376x460, clipsContent: true)
├── Rectangle "Background Image" (376x460, IMAGE fill — Unsplash, 커버와 동일 이미지)
├── Rectangle "Gradient Overlay" (376x460, GRADIENT_LINEAR)
│   stops: 0→rgba(0,0,0,0.66), 0.3→rgba(0,0,0,0.34), 0.52→rgba(0,0,0,0.16), 0.8→rgba(0,0,0,0.52), 1→rgba(0,0,0,0.82)
├── [상단 브랜드 칩] Rectangle "Brand Chip" (182x33, x:24 y:26, radius 20, 흰 16%+테두리 27%)
│   + Ellipse "Brand Dot" (7x7, #ffd200) + Text "ALL THAT CSR 매거진" (12.5px Black, #fff)
├── Text "Headline 1" (x:24 y:92, 35px Black, #FFFFFF, ls -1.3, drop-shadow)
├── Text "Headline 2" (x:24 y:140, 35px Black, #ffd200, drop-shadow)
├── [서브 박스] Rectangle "Sub Box" (300x58, x:24 y:214, radius 12, 흰 12%)
│   + Text "Sub Copy" (x:38 y:224, 13px Medium, #fff 85%, lh170)
├── [하단 발행칩] Rectangle "Vol Chip" (66x30, x:24 y:404, radius 20) + Text "Vol.[호수]" (11.5px Black)
└── [하단 발행칩] Rectangle "Date Chip" (132x30, x:98 y:404, radius 20) + Text "[YYYY년 M월 N주차]" (11.5px Bold)
```
※ 상단 3색 컬러바 없음(카카오풍). 모든 칩 radius 20, 반투명 흰 배경.

### 폰트
- Figma는 나눔스퀘어라운드 로드가 불안정할 수 있어 **Noto Sans KR(Black/Bold/Medium)로 대체** 렌더 가능 (HTML 썸네일은 나눔스퀘어라운드 사용).
- `await figma.loadFontAsync({ family: "Noto Sans KR", style: "Black" })` 등으로 로드 필수

### 배경 이미지 업로드
1. Bash로 Unsplash 이미지 다운로드: `curl -s -o /tmp/thumb_bg.jpg "[URL]"`
2. `upload_assets`로 업로드 URL 발급 (nodeId에 Background Image 노드 지정)
3. `curl -s -X POST "[submitUrl]" -F "file=@/tmp/thumb_bg.jpg;type=image/jpeg"`로 업로드
